class PetsController < ApplicationController
  ...
  def index
    ...
    @pet = if params[:page_set] == 'my_pet' # 自分のペットを表示
             @customer.pets.page(params[:page]).per(3).order('created_at DESC')
           else
             Pet.page(params[:page]).per(3).order('updated_at DESC') # ページング機能
           end
  end
